...
public class SQLProjManage {
  ...
  public Response addProject(@FormParam("PROJ_CD") String proj_cd,
    ...
    if (TextUtils.isBlank(proj_cd)
        || TextUtils.isBlank(proj_nm)
        || TextUtils.isBlank(proj_sdate)
        || TextUtils.isBlank(proj_edate)
        || TextUtils.isBlank(dept_cd)) {
  ...
  public Response getEmployees() throws SQLException {
    ...
    Connection connection = getSQLConnection();
    String sql =
        "SELECT PD.*, UI.USER_NM, UI.STATS,PI.PROJ_NM FROM PROJ_DETAIL PD, USER_INFO UI, PROJ_INFO PI WHERE PD.USER_ID = UI.USER_ID AND PD.PROJ_CD = PI.PROJ_CD";
  ...
  @Produces("application/json")
  @Path("/delEmployee")
  public Response delEmployee(@FormParam("PROJ_CD") String projCode,
